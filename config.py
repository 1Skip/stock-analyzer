...
CACHE_TTL_CORPORATE_ACTIONS = int(os.getenv("CACHE_TTL_CORPORATE_ACTIONS", "86400"))
CORPORATE_ACTION_FETCH_TIMEOUT_SECONDS = float(
    os.getenv("CORPORATE_ACTION_FETCH_TIMEOUT_SECONDS", "12")
)
...
T1_PLAN_PREHEAT_KLINE_MAX_WORKERS = int(os.getenv("T1_PLAN_PREHEAT_KLINE_MAX_WORKERS", "32"))
...
T1_PLAN_STRATEGY_TIMEOUT_SECONDS = float(os.getenv("T1_PLAN_STRATEGY_TIMEOUT_SECONDS", "1200"))
T1_PLAN_MULTI_FACTOR_TIMEOUT_SECONDS = float(os.getenv("T1_PLAN_MULTI_FACTOR_TIMEOUT_SECONDS", "3600"))
T1_PLAN_PREHEAT_JOB_TIMEOUT_SECONDS = float(os.getenv("T1_PLAN_PREHEAT_JOB_TIMEOUT_SECONDS", "7200"))
MULTI_FACTOR_DEEP_MAX_WORKERS = int(os.getenv("MULTI_FACTOR_DEEP_MAX_WORKERS", "12"))

# 统一交易账户。默认只观察实验策略，所有模拟成交必须由后续真实行情确认。
PAPER_TRADING_ENABLED = os.getenv("PAPER_TRADING_ENABLED", "true").lower() == "true"
PAPER_TRADING_RECONCILE_TIME = os.getenv("PAPER_TRADING_RECONCILE_TIME", "16:10")
PAPER_TRADING_INITIAL_CASH = float(os.getenv("PAPER_TRADING_INITIAL_CASH", "1000000"))
PAPER_TRADING_MAX_POSITIONS = int(os.getenv("PAPER_TRADING_MAX_POSITIONS", "5"))
PAPER_TRADING_POSITION_PCT = float(os.getenv("PAPER_TRADING_POSITION_PCT", "0.18"))
_PAPER_TRADING_STRATEGIES_RAW = os.getenv("PAPER_TRADING_STRATEGIES", "实验策略")
PAPER_TRADING_STRATEGIES = [
    item.strip() for item in _PAPER_TRADING_STRATEGIES_RAW.split(",") if item.strip()
]

# 组合级硬风控。百分比配置均使用 0-100 口径。
PORTFOLIO_MAX_DRAWDOWN_PCT = float(os.getenv("PORTFOLIO_MAX_DRAWDOWN_PCT", "12"))
PORTFOLIO_MAX_DAILY_LOSS_PCT = float(os.getenv("PORTFOLIO_MAX_DAILY_LOSS_PCT", "4"))
PORTFOLIO_MAX_INDUSTRY_EXPOSURE_PCT = float(
    os.getenv("PORTFOLIO_MAX_INDUSTRY_EXPOSURE_PCT", "40")
)
PORTFOLIO_MAX_ORDER_PARTICIPATION_PCT = float(
    os.getenv("PORTFOLIO_MAX_ORDER_PARTICIPATION_PCT", "0.5")
)
PORTFOLIO_MAX_DATA_AGE_DAYS = int(os.getenv("PORTFOLIO_MAX_DATA_AGE_DAYS", "5"))

# 券商适配默认禁用。只有三重闸门同时满足时，QMT 适配器才允许真实委托。
...
BROKER_RECONCILIATION_ENABLED = os.getenv(
    "BROKER_RECONCILIATION_ENABLED",
    "false",
).lower() == "true"
BROKER_RECONCILIATION_TIME = os.getenv("BROKER_RECONCILIATION_TIME", "16:15")
PORTFOLIO_BACKTEST_AUTO_ENABLED = os.getenv(
    "PORTFOLIO_BACKTEST_AUTO_ENABLED",
    "true",
).lower() == "true"
PORTFOLIO_BACKTEST_SCHEDULE_TIME = os.getenv("PORTFOLIO_BACKTEST_SCHEDULE_TIME", "16:20")
PORTFOLIO_BACKTEST_PERIOD = os.getenv("PORTFOLIO_BACKTEST_PERIOD", "1y")
