...
- **速度优化** — 历史 K 线使用本地交易日缓存，资金流短时缓存，财务/公告/研报按天缓存；页面提供“刷新数据”按钮用于手动刷新策略缓存。实时行情只用于计划内入场检查和最新价展示，不参与原始选股。
...
策略页会展示真实阶段进度：股票池、市值过滤、K 线轻筛、深度检查和完成状态。历史 K 线默认使用本地缓存并按交易日更新；实时行情不参与原始选股，只在 T+1 计划生成后用于计划内股票的盘中入场检查。需要强制更新策略缓存时点击页面上的“刷新数据”按钮。
