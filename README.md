...
- **推荐规则** — 热门板块保留全市场；智能推荐和推荐股推送仅推荐沪深主板；四板块固定为算力租赁、电力、苹果概念、特斯拉概念。
...
- 自选股摘要、四板块推荐股和每日完整日报都会同步推送“交易计划卡片 + 风控防御看板 + 资金博弈溯源”，与网页端个股分析口径保持一致。
...
- PEG 只有在 PE 和一致预期 EPS 增速同时可用时才展示；Beta 需要指数收益序列，当前个股页没有足够数据时显示“暂无”，不使用假基准。
