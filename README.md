...
- **个股决策盘后稳定性** — A 股个股分析日 K 缓存仅在交易时段按分钟刷新；同一交易日盘后复用 `YYYYMMDD-closed` 版本，避免盘后重复搜索同一股票时因日 K 缓存 key 分钟变化导致决策分、置信度和 Agent 结论不必要漂移。新增决策稳定性回归包，覆盖同输入重复决策一致、盘后缓存版本一致和盘中分钟刷新。
