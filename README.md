...
- **推送同步** — 15:45 统一 T+1 推荐计划覆盖短线/长线四板块，以及激进突破型/多因子稳健型全市场；所有策略只使用真实行情、资金、财务、风险事件等公开数据，缺失项会标注为缺失/接口失败/未通过。
...
- **飞书/Actions** — 支持 GitHub Actions 工作日 15:30 云端推送自选股摘要 + 大盘/完整日报，15:45 云端推送统一 T+1 推荐计划；飞书/企业微信 Webhook 可选。
...
- **推送内容补齐** — 自选股摘要、T+1 推荐计划和完整日报均会输出“交易计划卡片 + 风控防御看板”；一键测试推送只用于验证 Webhook 连通性。
...
- 15:30 定时推送只负责自选股摘要 + 大盘/完整 Markdown 日报，不再生成固定四板块推荐。
- 15:45 定时推送统一 T+1 推荐计划：短线/长线覆盖苹果概念、特斯拉概念、电力、算力租赁四板块；激进突破型/多因子稳健型只生成全市场 `全部`，默认每组 5 只。
- 自选股摘要、T+1 推荐计划和每日完整日报都会同步推送“交易计划卡片 + 风控防御看板 + 资金博弈溯源”；自选股摘要和 T+1 推荐计划还会展示主力吸货公式指标，与网页端个股分析口径保持一致。
...
定时推送复用 `scheduler.py`：配置 `NOTIFY_CHANNELS` 和对应 webhook 后运行 `python main.py --schedule`，每天 `SCHEDULE_TIME` 默认 15:30 执行自选股摘要 + 每日完整 Markdown 日报；`T1_PLAN_SCHEDULE_TIME` 默认 15:45 执行统一 T+1 推荐计划并推送。T+1 计划中，短线/长线覆盖苹果概念、特斯拉概念、电力、算力租赁四板块，激进突破型/多因子稳健型只生成 `全部`；短线/长线沿用沪深主板口径，激进突破型/多因子稳健型扫描沪深主板 + 创业板，科创板、北交所、ST、退市/异常股票不进入推荐池；热门板块页的行业板块、概念板块、个股涨跌幅榜保留全市场，不做主板过滤。配置页一键测试推送仅用于测试 Webhook 连通性。若本地调度器因断电、退出或异常留下旧 `.cache/scheduler.instance.lock`，需要先清理旧锁并重启 `python main.py --schedule`；缓存写锁残留会由 `data/cache.py` 自动识别并清理。若只想保存日报不推送正文，可设置 `DAILY_REPORT_PUSH_ENABLED=false`。

GitHub Actions 已启用工作日北京时间 `15:30` 和 `15:45` 两个定时点：15:30 运行 `.github/workflows/daily_analysis.yml` 的每日摘要/完整日报任务，15:45 运行统一 T+1 推荐计划任务，默认推送到飞书。配置仓库 Secret `FEISHU_WEBHOOK_URL` 后，不需要本地电脑常开；如需改为企业微信，把仓库 Variable `NOTIFY_CHANNELS` 设为 `wechat` 并配置 Secret `WECHAT_WEBHOOK_URL`；如需两个渠道同时推送，设为 `feishu,wechat` 并配置两个 Webhook。云端日报如需包含自选股，再配置 `STOCK_LIST` Secret（如 `600519,600036`，也支持 `捷顺科技,瑞鹄模具,上海电力`）或高级格式 `WATCHLIST_JSON`。如需云端 LLM 多空辩论，再配置 Secret `AI_API_KEY`，并把仓库 Variables 里的 `AI_DEBATE_ENABLED` 设为 `true`。也可以在 Actions 页面手动点击 `Run workflow` 选择 `daily` 或 `t1-plan` 立即测试。详细步骤见 `docs/FEISHU_GITHUB_ACTIONS.md`。
