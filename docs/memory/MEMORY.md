...
- [Bug fixes & context 2026-08-06](bug_fixes_and_context_2026-08-06.md) — 按用户要求取消本机 Windows 开机自启动，并删除每周一 09:30 的 Codex“短线闭环学习阶段检查”自动任务；核验无同项目注册表启动项、Windows 计划任务或服务，项目代码与策略语义未改。
- [Bug fixes & context 2026-08-02](bug_fixes_and_context_2026-08-02.md) — 真实巨潮资讯公司行为已接入统一账本和组合回测，现金分红/送转幂等入账、配股只告警不自动认购；普通策略超时提高到 1200 秒，4403 只股票和 9 个目标的真实 T+1 预热全部完成，并通过 1119 项完整断网测试。
- [Bug fixes & context 2026-07-27](bug_fixes_and_context_2026-07-27.md) — 实验策略 v3 同时接入成交量、成交额、换手率及20日相对强度，完成五年真实研究、统一模拟账户版本追踪和30笔后自动淘汰/合格候选切换；当前没有正式合格规则，市场广度不通过时保持现金；修复有效空仓计划被页面误判为缓存为空。
- [Bug fixes & context 2026-07-26](bug_fixes_and_context_2026-07-26.md) — 完成统一订单状态、审计账本、组合回测、硬风控、QMT 闸门、交易工作台和上线门禁；Baostock 逐日状态与历史日 K 已通过数据门禁；T+1 预热改为独立 worker 并复用内存 K 线，9 个目标真实运行成功；补充股票与 ETF 研究仍未通过成本、滚动和样本外门槛，策略继续不转正。
- [Bug fixes & context 2026-07-25](bug_fixes_and_context_2026-07-25.md) — 实验策略升级为市场广度过滤的趋势回撤止跌，新增可复现候选研究与统一模拟账户，修复全量股票池和一字跌停退出，确认当前真实市场应空仓且没有虚假成交。
