...
这个方案用于“电脑关机也能自动推送”。GitHub 会在工作日北京时间 15:30 云端推送自选股摘要 + 大盘/完整日报，并在 15:45 推送统一 T+1 推荐计划。
...
| `AI_MODEL` | 可选，默认 `deepseek/deepseek-v4-pro` |
...
- 工作日北京时间 15:30 自动运行每日摘要 + 完整日报。
- 工作日北京时间 15:45 自动运行统一 T+1 推荐计划。
- GitHub Actions 页面手动点击 `Run workflow`，选择 `daily` 或 `t1-plan` 立即测试。
...
1. 自选股摘要。
2. 每日完整 Markdown 决策日报。

15:45 运行会推送统一 T+1 推荐计划：

1. 短线/长线：苹果概念、特斯拉概念、电力、算力租赁四板块。
2. 激进突破型/多因子稳健型：全市场 `全部`，默认每组 5 只。

推荐股推送仅使用真实数据；热门板块排行仍保留全市场观察。
