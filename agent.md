...
- **回复纪律**：默认结论先行，不铺垫；日常问题 1-3 句，只答所问。除非用户明确说“展开 / 细讲 / 写方案 / 留档 / 改代码 / run / 端到端”，否则最多给 3 条可执行动作，不展开完整方案、备选路线或内部过程。
- **讲人话硬约束**：像懂行的朋友当面讲，不像写文档或客服话术。是/否、能/不能类问题先给结论，再补 1-2 句理由；默认不说文件路径、命令名、英文缩写或系统内部叫法，除非它们是解决问题必须的信息。
- **默认交付四件事**：结论（成了没、值不值、离目标近了多少）、功能（现在能干嘛、和以前差在哪）、怎么连（几块怎么串、失败会怎样）、下一步（用户拍板什么，或我直接做）。
- **禁止话术与冗余**：不要用“好的，我帮您...”“首先 / 其次 / 综上所述”式开场；避免“赋能、深度、维度、全链路、生态、助力”等空泛词。信息多不等于有用；用户说“太长”或“听不懂”时，视为上一版失败，立刻收成短句，只保留结论和下一步，不重述原答案。
...
| `notification.py` | 通知与报告文本 | `build_analysis_report()` 构造分析报告 |
...
- **日报调度**：`scheduler.py` 在 15:30 生成自选股摘要 + 完整 Markdown 日报；默认不跑推荐股扫描，`DAILY_REPORT_INCLUDE_RECOMMENDATIONS` 为 `false` 时避免和 T+1 预热重复扫描。
...
- **通知报告**：`notification.py` 负责构造本地分析报告文本，正式推送由调度/外部工作流按配置触发。
...
- **调度结论必须验证**：回答 scheduler 是否运行时，必须检查 `SCHEDULE_ENABLED`、进程/锁文件、状态缓存和最近日志，不能只按代码默认值判断。
...
- 作用：提醒 Codex 在本项目中遵守真实数据、推荐/T+1 边界、验证纪律和更新推送流程。
- 分工：`agent.md` 是项目红线、架构约定和长期协作规则的正式来源；Codex Skill 不替代 `agent.md`，只负责在 Codex 工作时强化执行这些规则。
- 后续处理推荐、调度、Feishu/GitHub Actions、缓存和 T+1 问题时，优先按 stock-analyzer skill 的 SOP 执行。
...
- **T+1 计划边界**：T+1 推荐计划只提前生成既有策略结果与交易计划，不改变选股、评分、排序或实时入场检查边界。
...
- **本地调度**：支持工作日 15:30 生成自选股摘要 + 完整 Markdown 日报，15:45 预热 T+1 推荐计划。
- **飞书/Actions**：飞书推送和 GitHub Actions 使用 T+1 计划与 Markdown 日报输出，按环境变量配置启用。
- **推荐边界**：热门板块保留全市场；普通短线保留沪深主板且只使用`全部`，短线经典版保留沪深主板和`全部、苹果概念、特斯拉概念、电力、算力租赁`五个板块并只做纯技术筛选；15:45 T+1 计划中普通短线只生成`全部`，经典版按`全部`加四个配置板块生成，激进突破型/多因子稳健型只生成全市场`全部`。
- **自选股同步**：本地自选股保存在 `watchlist.json`；云端工作流优先使用 Secret 中的自选股配置。
- **中文名称识别**：A 股中文名称搜索支持全量索引 + 本地缓存 + 静态兜底，避免“报一个补一个”。
...
- 需要同步到 GitHub 时，先确认文档、测试和敏感文件状态。
- 后续继续保持 Web、CLI、调度和 T+1 计划口径一致。
