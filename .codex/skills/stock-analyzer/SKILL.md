...
- For current recommendation UI: normal `短线` only shows `全部`; `短线经典版` shows `全部, 苹果概念, 特斯拉概念, 电力, 算力租赁` and keeps pure technical semantics; aggressive breakthrough and multi-factor steady only show/use `全部` for T+1 plan generation.
